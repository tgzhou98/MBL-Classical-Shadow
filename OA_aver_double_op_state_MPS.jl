...
function OAnorm_for_averageOA(t_arr::Array{Float64, 1}, k_arr::Array{Int64, 1}, Delta::Float64, h_arr::Array{Float64, 1}, nsite::Int64, W_disorder::Float64, Nrealization::Int64,
    maxdim_val::Int64, cutoff_val::Float64, file_extension::String)

    ttotal = last(t_arr)
    total_dirname = "OA_average_tJ$(ttotal)"
    ...
    # Here we don't specify the position of OA, but only fixed the total length to be k
    psi_target_klen_map = Int64[]
    k_len = length(k_arr)
    OA_weight_each_klen = [1/(nsite-k+1) for k in 1:k_len]
    for k in 1:k_len
        for aver_ind in 1:nsite-k+1
            targetstate = ["IIstate" for n=1:nsite]
            for ll in aver_ind:(aver_ind+k-1)
                targetstate[ll] = "Occstate"
            end
...
function run_OA_sh_fixedOA()
    nsite = 50                       # Site Number
    cutoff_val = 1E-28               # cutoff for shadow norm MPS
    maxdim_val = 100                 # The maximal bond dimension for the shadow norm calculation
    tau = 0.1                        # Trotter decomposition time step
    ttotal = 6.0                     # Total evolution time of MBL Hamiltonian
    ...
    for W_disorder in W_disorder_arr
        ...
        OA_sh_t_all, h_arr = OAnorm_for_averageOA(t_arr, k_arr, Delta, h_arr, nsite, W_disorder, Nrealization, maxdim_val, cutoff_val, file_extension)
    end
